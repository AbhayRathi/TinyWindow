...
def test_rust_encryption_error_handling():
    ...
    assert tinywindow.verify(key2, payload, sig) is False, "Signature with wrong key should fail"
    
    # Test: tampered signature fails
    sig_tampered = bytearray(sig)
    sig_tampered[0] ^= 0xFF
    assert tinywindow.verify(key1, payload, bytes(sig_tampered)) is False, "Tampered signature should fail"
    
    # Test: empty payloads work
    empty_payload = b""
    sig_empty = tinywindow.sign(key1, empty_payload)
    assert tinywindow.verify(key1, empty_payload, sig_empty) is True, "Empty payload should work"
...
def test_rust_encryption_security():
    ...
    assert tinywindow.verify(key_b, payload, sig_a) is False, "Signature should not verify with different key"
