...
mod tests {
    ...
    fn test_sign_deterministic() {
        ...
        assert_eq!(
            sig1, sig2,
            "sign must be deterministic for the same key and payload"
        );
        assert_eq!(sig1.len(), 32); // HMAC-SHA256 produces 32-byte signatures
    }
    ...
    fn test_verify_fails_with_wrong_payload() {
        ...
        assert!(
            !verify(&key, payload2, &sig),
            "verify should fail with wrong payload"
        );
    }
